...
//Get all the signatures... well, the most recent 100...
app.get("/signatures", async (req, res) => {
  const queryResults = await client.query("select * from signatures order by time desc limit 100");
  const signatures = queryResults.rows;
  res.status(200).json({
    status: "success",
    data: {
      signatures
...
app.get("/signatures/:id", async (req, res) => {
  ...
  const queryResults = await client.query("select * from signatures where id = $1 limit 1", [id]);
  const signature = queryResults.rows;
...
app.post("/signatures", async (req, res) => {
  ...
  if (typeof name === "string") {
    const result = await client.query("insert into signatures (signature, message) values ($1,$2) returning *", [name, message])
    const createdSignature = result.rows[0];
    res.status(201).json({
      status: "success",
      data: {
...
app.put("/signatures/:id", async (req, res) => {
  ...
  if (typeof name === "string") {
    const result = await client.query("update signatures (signature, message) values ($1,$2) where id = $3", [name, message, id])
...
app.delete("/signatures/:id", async (req, res) => {
  ...
  const queryResult = await client.query("delete from signatures where id = $1", [id]);
